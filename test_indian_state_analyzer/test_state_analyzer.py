...
def test_read_state_census_csv_data_should_return_number_of_records_in_file():
    assert StateAnalyzer.read_state_census_csv_data(state_census_data_file) == 29
...
def test_read_state_census_csv_data_when_given_inappropriate_file_should_raise_CsvReaderException(my_file, error_message):
    with pytest.raises(CsvReaderException) as csv_error:
        StateAnalyzer.read_state_census_csv_data(my_file)
    assert csv_error.value.message == error_message

def test_read_state_code_csv_data_should_return_number_of_records_in_file():
    assert StateAnalyzer.read_state_census_csv_data(state_code_data_file) == 37

@pytest.mark.parametrize(
    'my_file, error_message', [
        (wrong_delimiter_file, ExceptionType.IncorrectDelimiterException),
        (wrong_type_file, ExceptionType.IncorrectFileTypeException),
        (state_census_data_file, ExceptionType.IncorrectCsvHeaderException),
        (us_census_data, ExceptionType.IncorrectCsvHeaderException),
        (absent_file, ExceptionType.IncorrectFilePathException)
    ]
)
def test_read_csv_data_should_return_number_of_records_in_file(my_file, error_message):
    with pytest.raises(CsvReaderException) as csv_error:
        StateAnalyzer.read_state_code_csv_data(my_file)
    assert csv_error.value.message == error_message